...
class SignAndVerify extends Component {
  ...
  signMessage() {
    ...
    this.props.wallet.accounts.forEach((a, index) => {
      let tmp = a.addresses.chains[0].find(rvnbox.Address.toLegacyAddress(this.props.signAndVerify.signAddress))
      if(tmp !== undefined) {
        account = a;
        addressHeight = tmp;
      }
    ...
    try {
      hdNode = rvnbox.HDNode.fromXPriv(account.xpriv);
      childNode = hdNode.derivePath(`0/${addressHeight}`);
      privateKeyWIF = rvnbox.HDNode.toWIF(childNode);
    }
    catch (e) {
      this.props.updateSignAndVerifyValue('message1Success', '');
    ...
    let signature = rvnbox.RavenCoin.signMessageWithPrivKey(privateKeyWIF, this.props.signAndVerify.message1);
  ...
  verifyMessage() {
    ...
    let error = false;
    try {
      address = rvnbox.Address.toLegacyAddress(this.props.signAndVerify.verifyAddress);
    }
    catch (e) {
      error = true;
      this.props.updateSignAndVerifyValue('message2Success', '');
    ...
    let verified;
    try {
      verified = rvnbox.RavenCoin.verifyMessage(address, signature, message)
    }
    catch (e) {
      error = true;
      this.props.updateSignAndVerifyValue('message2Success', '');
